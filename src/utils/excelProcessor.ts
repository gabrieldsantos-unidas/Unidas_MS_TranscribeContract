    ...
    reader.onload = (e) => {
      try {
        ...
        if (isPoolDePneus) {
          ...
          resolve([
            { tableName: 'PoolDePneus', data: allPoolRows },
            { tableName: 'PoolDePneusItem', data: allPoolItemRows }
          ]);
          return;
        }
        ...
        const transformedData = jsonData.map((row: any, rowIndex: number) => {
          ...
          if (newRow['TipoSeguroCasco'] !== undefined) {
            if (newRow['TipoSeguroCasco'] === 'Apólice Contratada pela Locadora'){
              newRow['TipoSeguroCasco'] = '1';
            }
          }
